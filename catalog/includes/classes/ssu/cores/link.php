...
*/
	class SSULink{
		...
		public function parseURL(){
			...
			elseif(substr($this->original_uri, 0, 4) == 'blog'){
				return false;
			}
			...
			if(empty($this->original_uri)){
				...
				if(SSUConfig::registry('configs', 'alias_status')){
					...
					if(($alias_get_array = SSUCache::read($this->alias_filename, 'aliases', true)) !== false){
        ...
        elseif(!empty($this->alias_filename) && !$this->checkPageExcludedList($_GET['main_page']) && !isset($_GET[zen_session_name()]))
            SSUCache::write($this->alias_filename, 'aliases', http_build_query($_GET), true);
    }  
		...
		function parseParams(&$languages_code, &$page, $parameters){
			...
			if($set_cache) SSUCache::write("{$cache_filename}_{$languages_code}", 'pc', $params, true); 
